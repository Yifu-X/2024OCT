'''
v3.5:横截面相对于镜头不动，但是失败了，实时渲染的刷新率不够，抖动非常明显，不好用
v3:点击显示横截面，读取现有模型，视窗显示
v2:只捕捉鼠标在模型上的坐标
v1:实现显示切片图像
'''
import time
...
def left_button_press_callback(obj, event):
    ...
    if picker.Pick(x, y, 0, ren):
        ...
        # slice_actor.SetPosition(512 * 2, -512 * 2, 0)
...
# 创建相机并设置位置
camera = vtkCamera()
camera.SetPosition(0, 0, 1000)  # 相机位置
camera.SetFocalPoint(0, 0, 0)  # 相机焦点
...
def update_image_position():
    ...
    cam_direction = [cam_focal[i] - cam_pos[i] for i in range(3)]  # 计算相机朝向

    # 将图像放置在相机前方，距离可以调节
    distance_from_camera = 5.0
    new_image_position = [
        cam_pos[i] + cam_direction[i] * distance_from_camera for i in range(3)
    ]

    # 更新图像的位置信息
    slice_actor.SetPosition(new_image_position)

    # 计算图像与相机保持一致的法线方向
    slice_actor.SetOrientation(0, 0, 0)  # 重置图像的角度
    camera_direction_norm = vtkMath.Normalize(cam_direction)  # 归一化方向向量
    slice_actor.RotateX(camera_direction_norm)  # 让图片与相机的方向对齐
...
# 绑定渲染回调函数
interactor.AddObserver('RenderEvent', render_callback)
