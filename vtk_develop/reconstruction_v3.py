...
from vtk import vtkSTLWriter,vtkPolyDataWriter
...
# 读取JPG序列
def jpgReader(filepath:str, width, height, star_num, end_num, spacing = [4, 4, 7]):
    ...
    JPG_Reader.SetDataExtent(0, width-1, 0, height-1, star_num, end_num)  # 读取PNG数据的空间范围，z范围代表图片序列数,用来定义要读取的图像序列的范围
    JPG_Reader.Update()
...
# 读取jpg序列
jpgmodel = jpgReader("interpolation_out_3d_in", 512, 512, 0, 1076,[4,4,7])
